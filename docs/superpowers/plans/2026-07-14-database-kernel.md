...
- [x] **步骤 1：编写 Provider 所有权和 Clock 失败测试**
...
- [x] **步骤 3：实现最小 Provider 状态、所有权和 Clock**
...
- [x] **步骤 4：编写 busy retry 和生命周期失败测试**
...
- [x] **步骤 1：把 SessionStore 测试改成受控 Provider 并新增 Clock/事务断言**
...
- [x] **步骤 2：运行 SessionStore 测试确认旧实现失败**
...
- [x] **步骤 3：实现 Provider 注入与 SessionStore 迁移**
...
- [x] **步骤 2：运行 lifecycle 测试确认缺失 Provider 阶段**
...
pnpm test
pnpm build
git diff --name-only main | xargs pnpm prettier --check
git diff --check
```

预期：所有任务门禁和本分支改动文件检查 exit 0，无失败或格式差异。全仓 `pnpm prettier --check .` 在最新 `main` 已有 106 个既存格式差异，不在本分支批量改写。

- [x] **步骤 4：规格逐项审计和主代理 review**
...
最终审计额外以 `COLORFUL_CODE_V2_ENABLED=false` 启动真实 daemon，验证 `/health`、`/plugins/installed`、`/sessions`，正常终止后重新获取同一数据目录的 Instance Lock。该 smoke test 发现并修复了 `SessionStore` 测试故障参数缺少 Nest 可选注入标记的问题，并增加完整 `AppModule` 依赖解析回归测试。

- [x] **步骤 5：保留最终改动供用户 review**

按用户要求，不提交最后的 review 记录和计划勾选。报告已提交的阶段性 commit、未提交文件、验证证据和仍需用户 review 的最终 diff；不 push、不创建 PR、不合并。
