...
> **状态：实现、安全收口、Phase 0A 全仓 Gate 与最终独立只读审查均已完成；尚未暂存或提交，保留给人工复核**
>
...
├─ migration-snapshot-read-only policy
...
- `migration-snapshot-read-only`：备份、恢复和完整性验证使用的窄只读连接；接受受控的 `WAL` 或独立快照的 `DELETE` journal header，但仍强制 foreign keys、`FULL`、`MEMORY`、`trusted_schema = OFF` 和 `query_only = ON`。
...
- 当前 Bun SQLite driver 是否提供 connection serialization；诊断记录实际选用的方法；
- SQLite `RETURNING` 语法所需版本，供后续 Phase 使用；
- 与安全边界相关的编译选项。

当前 Bun 1.3.14 暴露 `Database.serialize()`，并为每次调用返回独立 Buffer。迁移备份使用独立只读连接捕获包含 WAL 可见提交的快照，直接持有该 Buffer 以避免第二份数据库大小的复制，随后关闭快照连接；独立文件的 SQLite header 被规范化后再执行完整性与外键校验。自动快照在序列化前后都受 512 MiB 上限约束，不执行启动期 `VACUUM`。缺少 serialization capability 时明确拒绝启动。
...
`DatabaseProvider.close()` 首次调用立即将状态变为 `closing`，拒绝新的 read、transaction 和 clock 操作。关闭协调器只执行一次物理关闭，重复调用返回同一个 Promise。调用方可传入 `checkpointSignal`；若 signal 在 checkpoint 开始前已中止，Provider 记录真实 `interrupted` 结果后仍完成物理关闭和 ownership 释放。同步 SQLite PRAGMA 一旦开始不能被 JavaScript signal 强行打断，因此该 signal 是关闭阶段的有界 preflight，而不是伪装成可抢占的线程取消。

关闭业务读写连接前执行一次 `PRAGMA wal_checkpoint(PASSIVE)`。物理只读连接不产生 WAL 写入，且 SQLite readonly handle 无法执行 checkpoint，因此只读 Provider 跳过 checkpoint，直接在 active read drain 后关闭句柄。读写结果结构化为：
...
SQLite 的 `log = -1, checkpointed = -1` 表示数据库不处于 WAL 或 checkpoint 未运行，不能归一化成“0 frame completed”。统一 policy 下该结果是配置/运行时不变量破坏，必须明确失败。真正的空 WAL 返回 `0/0 completed`。
...
Provider 使用 active-operation lease 协调关闭：`close()` 首次调用立即拒绝新操作，但必须等待已进入的 read 或真实 pending transaction attempt/retry 离开后再 checkpoint 和物理 close。read、transaction callback 或 Database Clock 内重入调用 `close()` 只获得一个待完成 Promise，不能同步关闭当前连接；正在 retry sleep 的 transaction 不得开始下一次 attempt，等待期间第二个 transaction 仍以 concurrent/nested error 拒绝。同步 COMMIT 或 ROLLBACK 已完成后立即释放 transaction ownership，不额外占用一次 Promise consumer microtask，否则会把 1.x 紧随其后的同步持久化误判为并发事务。

Provider 的 path preparation、database open 和 ownership conflict 使用稳定 typed error；公开 message、字段、序列化和 cause 不包含原始或 canonical 数据目录、SQLite URI、token 或 credential。内部 ownership map 仍用 canonical path 做正确排他，但不向错误边界泄漏。

启动和关闭均禁止自动执行 `VACUUM`。Instance Lock 使用固定 `application_id` header write 初始化零字节锁文件；迁移备份和恢复使用有大小上限的 connection serialization。手工运维备份命令不属于 daemon 自动启动链。
...
`TestDatabaseClock` 实现生产 `DatabaseClock`，以安全整数 epoch milliseconds 初始化，并允许测试通过同步 `set()` 和 `advance()` 控制时间。持久化时间仍必须由 transaction context 的 `now` 或 Provider clock 读取，测试不得把 wall clock 直接写入数据库。
...
- `empty`：不存在数据库文件，由完整启动链创建并返回已迁移 Provider；只有显式 `schemaVersion: 0` 返回未管理的空 Schema。
- `migrated`：已经应用当前 migration registry 的数据库。
- `legacy-1x`：当前正常 1.x fixture，可通过 `legacyVariant` 选择 normal、missing-optional、orphaned 或 corrupt-record 变体。
...
- `read-only`：连接以 SQLite 物理 readonly flag 和 Provider read-only role 打开；返回的 Provider 允许 read，transaction 明确拒绝。工厂不把 chmod 当作 WAL 安全边界。
...
- 在真实 Provider close、Instance Lock release 或目录删除已经成功后注入命名的 synthetic cleanup error，用于验证主体错误优先级而不故意泄漏资源。

checkpoint completed/incomplete/interrupted/throw 和真实 Provider close failure 由 Provider 的窄测试 seam 覆盖，不通过 Test Database Factory 暴露任意依赖 callback。
...
`restart()` 是测试专用的完整重启协调器：同步阻止新 raw/busy 资源，释放辅助连接，关闭 Provider，释放 Instance Lock，再为同一数据目录重新获取 Lock、执行无操作 migration 校验并创建新 Provider。任何一步失败都按 fail-closed 顺序保留或清理所有权；它用于联合 Gate 证明“完整关闭 → 释放锁 → 再次启动”，不是生产重启 API。

### 4.6 文件身份、权限与备份耐久性

Instance Lock 使用独立 guard lock 与主 lock 两个固定文件。两者拒绝 symlink、非普通文件和 `nlink != 1`，只接受空的 `application_id` 或项目固定值，并在 SQLite 打开前后复核 `dev/ino`。新数据目录为 `0700`，锁文件为 `0600`；对历史 1.x 常见的 `0755` 数据目录，启动时通过已打开的目录句柄收紧为 `0700` 并复核句柄与路径 identity，收紧失败才拒绝启动，从而同时满足本机隐私和升级兼容。主 lock 路径运行中被替换时，guard 仍阻止第二 daemon，`assertHealthy()` 与 release 返回明确错误。

迁移的 `backups`、`migration-quarantine` 和 staging 目录强制 `0700`，数据库快照、manifest 与 reservation 强制 `0600`。发布备份前依次 fsync 快照、manifest 和 staging 目录，rename 后 fsync 父目录；跨目录 quarantine 同步源父目录和全部目标目录，恢复 hard-link 发布同样在可见性变化前后同步文件和目录。备份 manifest 只保存数据库 basename，不持久化用户绝对路径。验证拒绝 symlink、hardlink 和验证期间 inode 变化；恢复消费还要求已验证 inode 在打开前后和 serialization 后保持一致。backup 前后通过同一 migration connection 的 `PRAGMA data_version` 检测并发提交，发现漂移时在任何 migration mutation 前失败且不覆盖新写入。
...
当前生产 1.x Schema 的权威来源是本分支开始时已发布建库逻辑的冻结副本，并通过真实 SQLite 建库后 introspection 验证。实现已将其移入只负责 baseline 的生产模块，使以下消费者共享同一份 canonical SQL：
...
- migration registry checksum/history：覆盖当前程序管理的 migration metadata；用于重启、顺序、name 和 checksum drift 验证。
...
WAL fixture 必须在第二条连接仍可观察 WAL 时提交数据并保留未 checkpoint frame。备份测试证明只复制 main database 文件会遗漏该数据，而受大小限制的 connection serialization 会包含该数据。manifest 记录 fixture 的预期 WAL 状态，但不把易变 frame 数纳入逻辑 checksum。
...
→ Provider 初始化时生成 SQLite capabilities diagnostics
→ 分类 empty / legacy 1.x / managed / newer / corrupt
→ 对 pending migration 创建一致性备份
→ 执行 migration 1 或后续 migration
→ integrity_check + foreign_key_check + legacy schema/data checksum + migration history checksum
→ 关闭 bootstrap connection
→ 创建并配置 Provider
```

正常 1.x 数据库 adoption 不运行 baseline DDL，也不改变 1.x 业务数据。migration 前后分别计算 `legacySchemaChecksum` 和各表 logical rows checksum，必须一致；migration framework 同时验证 metadata history、name 和 migration source checksum 与当前 registry 完全匹配。虽然 daemon 持有合作进程使用的 Instance Lock，adoption 事务仍必须复验，因为第三方 SQLite connection 不受该锁协议约束。

损坏的起始数据库、baseline manifest 不匹配或已应用 migration checksum 被修改时，在任何 migration mutation 前 fail-closed，不覆盖原库、不继续业务写入；此时没有可信的本次迁移前备份，不能伪装成已执行 restore。只有创建一致性备份后 migration 已开始且随后执行或 post-migration 验证失败，才执行 quarantine + atomic restore，恢复后仍以 `migration_failed_recovered` 退出。较新版本明确拒绝，不尝试 downgrade、覆盖 metadata 或继续业务写入。Test Database Factory 必须在自动清理前观察并证明注入的 migration failure 已完成该恢复闭环。
...
4. migration 前后 `legacySchemaChecksum` 不变，当前 migration history/checksum 正确。
...
2026-07-14 最终验证中，根目录 `pnpm lint`（11/11）、`pnpm typecheck`（16/16）、`pnpm build`（10/10）和 `pnpm test` 均以退出码 0 完成；完整测试中服务端 388 项、Web 45 项、CLI 5 项均为 0 失败。最后一个仅影响启动错误固定文案的防御补丁后，服务端 34 个测试文件再次以 388/388 通过，并重跑 server lint、typecheck 与根目录生产 build。最后一次沙箱外全仓复跑因工作区审批额度耗尽未获执行权限；其余 package 未被该补丁修改。完整测试中的既有 localhost 监听用例在允许绑定 `127.0.0.1` 的环境执行。所有本分支改动文件通过 Prettier 定向检查和 `git diff --check`。仓库级 `pnpm format` 仍会报告 106 个本分支未触碰的历史文件，故本分支不以批量格式化无关文件的方式掩盖既有基线问题。
