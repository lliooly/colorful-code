...
export function parseCliArgs(
  ...
  if (!prompt) {
    throw new CliUsageError('A prompt is required. Use --prompt "..." or pass it as positional text.');
  }

  const cwd = values.cwd ?? env.PWD ?? process.cwd();
  return {
    apiBaseUrl: stripTrailingSlash(
      values.apiBaseUrl ?? env.COLORFUL_CODE_API_BASE_URL ?? DEFAULT_API_BASE_URL
    ),
